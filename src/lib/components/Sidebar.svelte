...
<aside
	...
	style="border-color: var(--color-border); background: var(--color-surface); width: {collapsed
		? '0px'
	...
	<nav class="px-2 mt-2 flex-1 overflow-y-auto min-w-[240px]">
		<div class="px-3 py-1 flex items-center justify-between">
			<span
				...
				style="color: var(--color-text-muted);">Lists</span
			>
			<button
				onclick={() => {
					showNewList = true;
		...
		{#each $lists as list (list.id)}
			...
			{:else}
				<button
					...
					ondblclick={() => {
						if (list.id !== 'inbox') startRename(list.id, list.name);
					}}
					oncontextmenu={(e) => showContextMenu(e, list.id)}
					class="w-full text-left px-3 py-1.5 rounded-md text-sm flex items-center gap-2 transition-colors"
					style={$activeListId === list.id
						? 'background: var(--color-accent); color: white;'
	...
	>
		<button
			onclick={() => {
				...
				if (list && contextMenu) startRename(contextMenu.listId, list.name);
				closeContextMenu();
			}}
		...
		<div class="px-3 py-1.5">
			...
			<div class="flex gap-1 flex-wrap">
				{#each listColors as color (color)}
					<button
						onclick={() => {
							if (contextMenu) handleColorChange(contextMenu.listId, color);
						}}
		...
		<button
			onclick={() => {
				if (contextMenu) handleDeleteList(contextMenu.listId);
			}}
			class="w-full text-left px-3 py-1.5 text-sm hover:opacity-80"
