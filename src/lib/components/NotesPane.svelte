	...
	>
			...
			{#if editingSummary}
				<input
					...
					onkeydown={(e) => {
						...
						if (e.key === 'Escape') editingSummary = false;
					}}
					autofocus
				/>
			{:else}
		...
		<div class="flex-1 overflow-y-auto">
			...
			<div class="px-4 py-3 border-b" style="border-color: var(--color-border);">
				<label class="text-xs font-medium block mb-1.5" style="color: var(--color-text-muted);"
					>Priority</label
				>
				<div class="flex gap-1">
					{#each [{ value: 0, label: 'None', color: 'var(--color-border)' }, { value: 9, label: 'Low', color: 'var(--color-text-muted)' }, { value: 5, label: 'Med', color: 'var(--color-accent)' }, { value: 1, label: 'High', color: 'var(--color-danger)' }] as p (p.value)}
						<button
			...
			<div class="px-4 py-3 border-b" style="border-color: var(--color-border);">
				...
				<select
					...
					onchange={setRrule}
				>
					{#each rruleOptions as opt (opt.value)}
			...
			{#if todo.due}
				<div class="px-4 py-3 border-b" style="border-color: var(--color-border);">
					<span class="text-xs font-medium block mb-1.5" style="color: var(--color-text-muted);"
						>Reminders</span
					>
					{#each todo.reminders as reminder, idx (reminder.offsetMinutes)}
						<div
					...
					<div
						...
						onclick={(e) => {
							...
							if (target.tagName === 'INPUT' && target.getAttribute('type') === 'checkbox') {
								const checkboxes = (e.currentTarget as HTMLElement).querySelectorAll(
									'input[type="checkbox"]',
								);
